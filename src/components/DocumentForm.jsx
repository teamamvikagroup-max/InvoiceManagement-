...
export default function DocumentForm({ type, companies }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const recordRef = await saveInvoiceRecord({
        invoiceNumber,
        type,
        ...
        pdfUrl: "",
        pdfPath: "",
        pdfStatus: hasStorageBucket ? "uploading" : "local_only",
      });

      if (hasStorageBucket) {
        void uploadDocumentPdf(invoiceNumber, pdfBlob)
          .then(({ pdfUrl, pdfPath }) =>
            updateInvoiceRecordPdf(recordRef.companyId, recordRef.type, recordRef.id, { pdfUrl, pdfPath, pdfStatus: "uploaded" }),
          )
          .catch((uploadError) => {
            console.warn("Background PDF upload failed, falling back to local download.", uploadError);
            downloadPdfBlob(pdfBlob, filename);
            return updateInvoiceRecordPdf(recordRef.companyId, recordRef.type, recordRef.id, { pdfStatus: "local_only" });
          });

        setStatus({
          type: "success",
          message: `${type === "invoice" ? "Invoice" : "Quotation"} ${invoiceNumber} saved. We are uploading the PDF to Firebase Storage in the background so it can appear in History.`,
        });
      } else {
        downloadPdfBlob(pdfBlob, filename);
        setStatus({
          type: "success",
          message: `${type === "invoice" ? "Invoice" : "Quotation"} ${invoiceNumber} saved. PDF was downloaded locally because Firebase Storage is unavailable.`,
        });
      }
