...
export default function DocumentForm({ type, companies }) {
  ...
  const [pdfDocumentNumber, setPdfDocumentNumber] = useState("");
  const [pdfFinancialYear, setPdfFinancialYear] = useState("");
  ...
  const exportFinancialYear = pdfFinancialYear || previewFinancialYear;
  ...
  const submit = async (event) => {
    ...
    if (!selectedCompany || !pdfRenderRef.current) {
      setStatus({ type: "error", message: "Company details or PDF preview are not ready yet." });
      return;
    }
    ...
    try {
      ...
      setPdfFinancialYear(financialYear);
    ...
    <>
      ...
      <div className="pointer-events-none fixed -left-[200vw] top-0 opacity-0">
        <div ref={pdfRenderRef} data-pdf-template="PdfDocumentExportV3" data-pdf-logo-src={exportLogoSrc}>
          <PdfDocument
            type={type}
            invoiceNumber={exportDocumentNumber}
            financialYear={exportFinancialYear}
            dueDate={formData.dueDate}
