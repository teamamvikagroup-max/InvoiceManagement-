...
async function prepareCompanyForPdf(company) {
  ...
  if (company.logoBase64 || !company.logoUrl || company.logoUrl.startsWith("data:image/")) {
    return {
      ...company,
  ...
  try {
    ...
    return { ...company, logoUrl: dataUrl || company.logoUrl, logoBase64: dataUrl || company.logoBase64 || "" };
  } catch (error) {
    console.warn("[PDF Export] logo preload failed", error);
    return { ...company, logoUrl: company.logoUrl, logoBase64: company.logoBase64 || "" };
  }
}
...
export default function DocumentForm({ type, companies }) {
  ...
  const customerChange = (field, value) => setFormData((current) => ({
    ...current,
  ...
  const submit = async (event) => {
    ...
    if (!selectedCompany) {
      setStatus({ type: "error", message: "Company details are not ready yet." });
      return;
    }
    ...
    try {
      ...
      const pdfBlob = generatePdfBlob(pdfPayload, filename);
      ...
      const historyPayload = {
        ...
        companySnapshot: buildCompanySnapshot(preparedCompanyForPdf),
        customer: formData.customer,
        items: preparedItems,
        dueDate: formData.dueDate,
  ...
  if (!companies.length) {
    return (
      <EmptyState
        title="Add a company before creating documents"
        description="Invoices and quotations pull branding and GST details from the company profile section. Create at least one company to continue."
      />
    );
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-brand-700">
              {type === "invoice" ? "Invoice Builder" : "Quotation Builder"}
            </p>
            <h2 className="mt-2 text-2xl font-semibold text-slate-900 md:text-3xl lg:text-4xl">
              {type === "invoice" ? "Create a new invoice" : "Create a new quotation"}
            </h2>
          </div>
          <div className="rounded-2xl bg-slate-100 px-4 py-3 text-sm text-slate-600">
            Next number: <span className="font-semibold text-slate-900">{nextNumber || "Loading..."}</span>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-8">
          ...
          <section>
            ...
            <div className="mt-4 grid gap-5 md:grid-cols-2">
              <div>
                <label className="field-label">Select Company</label>
                <select
                  required
                  value={formData.companyId}
                  onChange={(event) => rootChange("companyId", event.target.value)}
                  className="input-field"
                >
                  <option value="">Choose a company</option>
                  {companies.map((company) => (
                    <option key={company.firebaseId ?? company.id} value={company.id}>
                      {company.name}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) => rootChange("dueDate", event.target.value)}
                  className="input-field"
                />
              </div>
            </div>
          ...
          <section>
            ...
            <div className="mt-4 grid gap-5 md:grid-cols-2">
              {[
                ...
                ["zipCode", "Zip Code", "text", false],
                ["placeOfSupply", "Place of Supply", "text", false],
                ...
                <div key={field}>
                  ...
                  <input
                    ...
                    onChange={(event) => customerChange(field, event.target.value)}
                    className="input-field"
                  />
                </div>
              ))}
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  onChange={(event) => customerChange("address", event.target.value)}
                  className="input-field"
                />
              </div>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between gap-4">
              ...
              <p className="text-sm text-slate-500">Add as many line items as you need.</p>
            </div>
            <div className="mt-4">
              <ItemsTable
                items={formData.items}
                onChange={(items) => rootChange("items", items)}
                onAddItem={() => rootChange("items", [...formData.items, createEmptyItem()])}
                onRemoveItem={(index) => rootChange("items", formData.items.filter((_, itemIndex) => itemIndex !== index))}
              />
            </div>
          </section>

          <section>
            <h3 className="section-title">Tax & Payment</h3>
            <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_320px]">
              <div className="space-y-5">
                <div>
                  <label className="field-label">GST Type</label>
                  <select value={formData.taxType} onChange={(event) => rootChange("taxType", event.target.value)} className="input-field">
                    <option value={TAX_TYPES.CGST_SGST}>CGST 9% + SGST 9%</option>
                    <option value={TAX_TYPES.IGST}>IGST 18%</option>
                  </select>
                </div>
                {type === "invoice" ? (
                  <div>
                    <label className="field-label">Payment Made</label>
                    <input
                      type="number"
                      min="0"
                      step="0.01"
                      value={formData.paymentMade}
                      onChange={(event) => rootChange("paymentMade", Number(event.target.value))}
                      className="input-field"
                    />
                  </div>
                ) : null}
                <div>
                  ...
                  <textarea rows="4" value={formData.notes} onChange={(event) => rootChange("notes", event.target.value)} className="input-field" />
                </div>
                <div>
                  <label className="field-label">Terms & Conditions</label>
                  <textarea rows="5" value={formData.terms} onChange={(event) => rootChange("terms", event.target.value)} className="input-field" />
                </div>
              </div>
              ...
              <div className="rounded-3xl bg-slate-950 p-6 text-white">
                ...
                <div className="mt-5 space-y-3 text-sm text-slate-200">
                  <div className="flex justify-between gap-4"><span>Subtotal</span><span>{formatCurrency(totals.subtotal)}</span></div>
                  <div className="flex justify-between gap-4"><span>CGST</span><span>{formatCurrency(totals.cgst)}</span></div>
                  <div className="flex justify-between gap-4"><span>SGST</span><span>{formatCurrency(totals.sgst)}</span></div>
                  <div className="flex justify-between gap-4"><span>IGST</span><span>{formatCurrency(totals.igst)}</span></div>
                  <div className="flex justify-between gap-4 border-t border-white/10 pt-3 text-base font-semibold text-white"><span>Total Amount</span><span>{formatCurrency(totals.totalAmount)}</span></div>
                  {type === "invoice" ? (
                    <>
                      <div className="flex justify-between gap-4"><span>Payment Made</span><span>{formatCurrency(totals.paymentMade)}</span></div>
                      <div className="flex justify-between gap-4 text-base font-semibold text-brand-200"><span>Balance Amount</span><span>{formatCurrency(totals.balanceAmount)}</span></div>
                    </>
                  ) : null}
                </div>
      ...
      <div className="xl:hidden glass-card p-4 md:p-6">
        <h3 className="text-lg font-semibold text-slate-900 md:text-xl">Live PDF Preview</h3>
        <p className="mt-2 text-sm leading-6 text-slate-600 md:text-base">
          For the best preview experience, open this page on a tablet or desktop screen. The generated PDF will still use your current form values on mobile.
        </p>
      </div>

      <div className="hidden xl:block glass-card p-4 md:p-6">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-slate-900 md:text-xl">Live PDF Preview</h3>
          <p className="mt-1 text-sm text-slate-600 md:text-base">The generated PDF will use this template and current form values.</p>
        </div>
        <div className="max-h-[calc(100vh-14rem)] overflow-auto rounded-3xl border border-slate-200 bg-slate-100 p-4">
          <div className="mx-auto w-fit">
            <DocumentPreview
              type={type}
              invoiceNumber={nextNumber || "Preview"}
              financialYear={previewFinancialYear}
              dueDate={formData.dueDate}
